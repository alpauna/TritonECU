...
static int pinv[8] = {1,1,1,1,1,1,1,1};    // inputs idle HIGH (pull-ups)
static int outv[8] = {0};
struct Edge { int pin; int level; uint32_t t; };
static Edge edges[64];
static int  nedges = 0;
void pinMode(int, int) {}
int  digitalRead(int p) { return pinv[p]; }
void digitalWrite(int p, int v) {
    // raw log: the PIN_* names do not exist yet, so name the legs at print time
    if (outv[p] != v) { outv[p] = v;
        if (nedges < 64) edges[nedges++] = { p, v, g_ms }; }
}
...
static void press(uint32_t hold) { pinv[PIN_BUTTON]=LOW; tick(hold); pinv[PIN_BUTTON]=HIGH; tick(300); }
static std::string drain() {           // format and clear the edge log
    std::string s;
    for (int i = 0; i < nedges; i++) {
        char b[48];
        snprintf(b, sizeof b, "%s%s@%ums ", edges[i].pin == PIN_LEG_A ? "A" : "C",
                 edges[i].level ? "hi" : "lo", edges[i].t);
        s += b;
    }
    nedges = 0;
    return s;
...
static void check(const char* what, bool want_pulse, bool want_on) {
    std::string log_ = drain();
    bool ok = (!log_.empty() == want_pulse) && (relayOn == want_on);
...
int main() {
    ...
    for (int i = 0; i < 4; i++) { pinv[PIN_BUTTON]=LOW; tick(3); pinv[PIN_BUTTON]=HIGH; tick(3); }
    press(60);               check("bounce+press",  true,  true);   // exactly one
    pinv[PIN_ECU]=LOW; tick(60); pinv[PIN_ECU]=HIGH; tick(300);
    ...
    printf("%-16s %-24s coil pin=%d  %s\n", "held on", drain().c_str(),
           outv[PIN_LEG_A], outv[PIN_LEG_A] == HIGH ? "ok" : "FAIL");
    if (outv[PIN_LEG_A] != HIGH) fails++;
#endif
