...
| `cam_target` ✅ | Was briefly held: the "no wheel dependency" claim was wrong, since `cam_target` is built on `hub_od = max(wheel_bore + 16, 46)`. `wheel_bore` is now a caliper 31.75, so `hub_od` is 47.75 and it is clear |
