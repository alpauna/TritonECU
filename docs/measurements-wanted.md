...
| **Both sensor phases, statically — IF THE TIMING COVER EVER COMES OFF** ⭐⭐⭐ | `CKP_GAP_TO_TDC_DEG` **and** the cam-to-gap phase in one sitting, with no engine running, no timing light and no degree tape. The cover has never been off ([timing marks](1999-Ford-F150-4wd-5.42v/CrankAndCamPositionForTiming.png)), so this is a plan rather than a task — but it is much the cheapest way to get both, and the opportunity does not come round often | Set the engine to the chain marks: cams on their marks, crank dot at 6 o'clock. That is a known mechanical position. Then read where the 36-1 gap sits relative to the CKP sensor, and where the CMP trigger sits relative to its sensor. Both phases fall straight out |
...
| **OD — caliper it** ⭐⭐ **blocking a print run** | ✅ **171.45 is dead** — the whole-template photo reads **~13.5 squares, ~135 mm**, 25 % under the listing. It drives `wheel_bore`, `key_w`, `key_d`, the **base wheel slot** and the **sensor bolt holes**: ±7 mm of OD moves `y_ck` ±3.5 mm, past a 4.4 mm hole. **`base` and `wheel_hub` cannot be printed until this is a caliper reading** — everything else can | Calipers, five minutes, the wheel is on the bench. Also take **thickness** while there: `wheel_thk = 3.05` still comes from the listing that was wrong about everything else |
