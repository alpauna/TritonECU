...
| **OEM vs Dorman: do the gaps coincide?** ⭐⭐ | A review of 917-060 says its teeth do not match OEM — codes and rough running. We measured this one at **5.00°, exactly half a pitch** from the 0° the README believed OEM used | **Already rigged** — both wheels are on the crank on one key. Just look: do the two missing teeth line up? Any stagger *is* the difference, in tooth-widths. Compare ODs in the same look |
| **Genuine `XW1Z-12A227-AC` vs each of them** ⭐⭐ ⏳ **arriving 2026-09-22** | Both of the rows above, **separately**. Ford-vs-Dorman answers *is the review real* with the truck out of it; Ford-vs-truck-wheel answers *does a 1999 use this design* with the Dorman out of it | Stack on the key, two readings, either order — the key registers each wheel so a three-high stack is not needed. **Read the casting, caliper the OD and count the teeth first** |
| **`CKP_GAP_TO_TDC_DEG`, directly** ⭐⭐ ⏳ | The whole constant, wheel geometry *and* sensor mounting angle in one reading. **The front cover is currently off — this closes when it goes back on** | Piston stop on #1, bring it to TDC, read where the gap sits relative to the CKP sensor |
| ↳ **Do not wait for the Ford part to take it** | A swap invalidates **the number, not the measurement**: the sensor term belongs to the block and survives it, so `new = old + rim stagger`. **One piston stop, ever** | Measure now against whatever is fitted, then stack the incoming wheel to carry it across |
| **The casting number on the OEM wheel** | ◐ **Demoted to a cross-check.** It was load-bearing only because each wheel was the other's reference; with a genuine Ford ring in the stack, neither is. Still free, still worth reading | It is cast around the hub face; the wheel is in hand |
| **Does a 1999 5.4L even use this reluctor?** | ◐ **Argued, not measured.** The OE number `XW1Z-…` carries Ford's **1999** prefix (`X` = 1999, as `1` = 2001 on the donor PCM), revised to `-AC` and still cataloged for 2001-2010 — one design spanning 1999→2010. **That is a reading of a part number** | **Now directly falsifiable:** stack the genuine ring against the truck's own wheel. Coincide and the decode holds; stagger and 2001 is a real boundary — **and the Dorman's stagger was never a defect** |
