...
| **M1** ✅ | Storage + config | SD, JSON config, persistence | **Done on both targets.** ESP32-P4 (SDMMC) 2026-09-06; **STM32 (SPI4) 2026-09-07** — reads back the same card the P4 wrote |
| **M2** ✅ | Analog front end | AD7606 | **Done on both targets.** ESP32-P4 2026-09-06; **STM32 2026-09-07** — same two known signals, SPI_MODE2 correct on both. See [`adc-front-end.md`](adc-front-end.md) |
...
| **M11** | SCP bus | J1850 PWM | Reads the OEM PCM's traffic on the bench first, then the truck. **Hardware is on the v1 board** — DRV8837 + TLV7031; without it the truck has no tach, speedo or MIL |
...
```bash
pio test -e native          # decoder logic, no hardware
pio run -e selftest -t upload    # header shorts and bridges
pio run -e spisweep -t upload    # SPI mode against a known signal
pio run -e rangecheck -t upload  # is the RANGE pin connected
pio run -e noisefloor -t upload  # achieved noise floor and noise-free bits
```
