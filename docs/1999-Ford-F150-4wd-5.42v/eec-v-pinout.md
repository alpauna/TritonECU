...
| **64** | -- | -- | **Trans Pos Sensor 3 (TR3A)** — *and* 12 V in START | **LB/YE (199)** | ✅ **Both.** `4R70W-Shift-Connector.png` gives DTR pin 3 = `199 LB/YE` = **TR3A**. The DTR is also the **neutral safety switch**, so 199 is fed from the start circuit *through* the range switch — 12 V here means **cranking in Park or Neutral**. An earlier revision called this START *instead of* TR3A; that was wrong. [`schematic-findings.md`](schematic-findings.md) §21 |
...
- **TR sensor is a 4-bit digital code**, not an analog ladder — TR1/TR2/TR3A/TR4
  ...
  (`4R70W-Shift-Connector.png`); the bit count was never in doubt and an earlier
  revision of this line wrongly said otherwise — [`schematic-findings.md`](schematic-findings.md) §21.
