...
| **TR** — transmission range | **4-bit digital code** | Park/neutral/gear on pins **34/49/64/50** (TR1/TR2/TR3A/TR4), not an analog ladder — no ADC channel needed. ✅ Confirmed against DTR connector C182. Pin 64 also reads 12 V while cranking, because the DTR is the neutral safety switch — [`schematic-findings.md`](schematic-findings.md) §21 |
