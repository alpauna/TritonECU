...
### Drive capability confirmed — same for both pins
...
## 9. ENOUT — this should sequence the SEPIC, not go to a GPIO

> *An open-drain output that goes high impedance when the voltage at the OUT pin
> is above (V<sub>CC</sub> − 0.7 V), indicating the external MOSFETs are fully
> on. The state of the pin is **latched** and resets when the OUT pin drops
> below 2.2 V.*

**Connect ENOUT to the LM5155-Q1's EN pin.** This is a power-supply sequencing
signal, not a status flag — routing it to the MCU would be near useless, since
the MCU only exists once the SEPIC is already running and would always read
"good".

What it buys: **the SEPIC does not load Q1 during inrush.** Without it, the
converter starts drawing current through a partially-enhanced pass FET running
in linear mode — adding dissipation during exactly the event whose thermal
budget §6 spent pages on. With it, Q1 charges C_out into an unloaded rail, then
the SEPIC starts against a fully enhanced FET.

It also gives clean behaviour on every auto-retry cycle: OUT collapses below
2.2 V, ENOUT re-latches low, the SEPIC is disabled, and the LTC4364 retries into
capacitance alone rather than into capacitance plus a live converter.

### The latch is what makes this work at all

ENOUT asserts on `OUT > VCC − 0.7 V`, so the pass pair's drop has to be under
700 mV for it to release. Under load that is not guaranteed:

| Condition | Input current | Q1+Q2 drop | vs 0.7 V |
|---|---|---|---|
| At startup, SEPIC disabled | ~0 A | ~0 mV | ✓ asserts |
| Running, 13.8 V, typical drive | 0.9 A | 0.10 V | ✓ |
| 4.4 V floor, 12 W out, typical | 3.2 A | 0.36 V | ✓ |
| 4.4 V floor, 12 W out, **worst-case drive** | 3.2 A | **1.99 V** | **✗** |

The worst-case corner — max V<sub>th</sub>, min gate drive, minimum input, full
load — would hold ENOUT low and never let the SEPIC start.

**The latch removes the problem entirely.** ENOUT only has to release *once*, at
startup, when the SEPIC is still disabled and the load is microamps. After that
it stays high-impedance no matter how far the drop grows under load, until OUT
genuinely collapses below 2.2 V. That is not incidental behaviour — it is why
the latch is in the part.

### Pull-up rail: VIN (the LTC4364 OUT rail) — better than VCC

**Not 3.3 V** — that rail is generated by the SEPIC that ENOUT enables.

An earlier revision of this review recommended VCC. **VIN is better**, and the
reason is the objection I had raised against my own suggestion: VCC sits behind
R4, so pulling ENOUT up to it puts the pull-up current through the 220 Ω feed
that §2 shrank specifically to protect V<sub>CC</sub> headroom at the input
floor. **Pulling to VIN loads R4 not at all.**

The argument for VCC was that it is stable from power-up while OUT ramps. That
is not worth anything here: **ENOUT holds the node low until `OUT > VCC − 0.7 V`
anyway**, so by the moment the pull-up has a job to do, OUT is already at full
rail. The sequencing does not care.

VIN also puts the LM5155-Q1's EN divider on the rail EN is actually referenced
to, which is where it belongs.

### Sizing: 100 kΩ

ENOUT sinks up to 2 mA, and the highest rail it may have to pull down against is
the 27 V clamp — a key-on during an overvoltage, since ENOUT is latched
high-impedance at all other times OUT is that high:

```
R ≥ 27 V / 2 mA = 13.5 kΩ        absolute floor
```

**Use 100 kΩ** (270 µA at the clamp). Note that **10 kΩ — the correct value for
FLT# into a 3.3 V MCU — is wrong here**, drawing 2.7 mA and exceeding ENOUT's
sink rating.

### The pull-up is also the top of the EN divider

One network, two jobs. ENOUT ties to the junction, grounding it when asserted
and letting it divide when released:

```
VIN ──[ R_top 100 kΩ ]──┬── LM5155-Q1 EN
                        ├── ENOUT
                     [ R_bot 61.9 kΩ ]
                        │
                       GND
...
### The ratio is pinned, and that makes EN's rating the open question

The SEPIC must keep running down to the 3.9 V its input reaches at the 4.4 V
battery floor (§6), so EN has to stay above the ~1.26 V precision threshold
there. That fixes the ratio, and the ratio then fixes what EN sees at the top:

```
minimum ratio = 1.26 V / 3.9 V = 0.32
at the 27 V clamp, EN therefore sees ≥ 8.7 V — unavoidable

with 100 kΩ / 61.9 kΩ:
   3.9 V in  →  EN = 1.49 V   ✓
  13.8 V in  →  EN = 5.28 V
    27 V in  →  EN = 10.3 V   ← the number to check
```

**[verify] the LM5155-Q1 EN absolute maximum against ~10 V.** If EN is rated to
V<sub>IN</sub>, as many TI controllers are, nothing more is needed. If it is
lower, clamp EN with a small Zener rather than lowering the ratio — lowering it
would raise the SEPIC's own turn-off point above the cranking floor and give
back the low-line capability the whole topology was chosen for.

The divider does **not** need to provide input UVLO. ENOUT already gates the
converter, and the LTC4364's 4.47 V UV sets the system's cutoff.
...
### Pull-up rail: VCC works, and is better than OUT — but size it for R4

**Not 3.3 V** — that rail is generated by the SEPIC that ENOUT enables.

**VCC (post-R4) is the better of the two options.** Pulling to the LTC4364's OUT
rail also works, but OUT ramps during inrush and only reaches full voltage at
the moment ENOUT releases. VCC is stable from the instant the part powers up,
and R4/D3 already bound it.

The catch is that **while ENOUT is asserted low, the pull-up current flows
through R4** — the 220 Ω feed that §2 shrank specifically to protect V<sub>CC</sub>
headroom at the input floor:

| R<sub>pu</sub> | Sink at 27 V clamp | Current at 14 V | Extra drop across R4 |
|---|---|---|---|
| 10 kΩ | **2.7 mA** ✗ over the 2 mA rating | 1.4 mA | 308 mV |
| 47 kΩ | 574 µA | 298 µA | 66 mV |
| **100 kΩ** | **270 µA** | 140 µA | **31 mV** |

Two constraints set the floor on that resistor:

```
ENOUT sink limit    R ≥ 27 V / 2 mA = 13.5 kΩ      at the clamp voltage
R4 headroom         keep the added drop ≪ the 0.17 V already budgeted
```

**Use 100 kΩ.** A 10 kΩ pull-up — the value that is right for FLT# into a 3.3 V
MCU — would exceed ENOUT's 2 mA sink rating during a clamp event and cost
308 mV across R4 at exactly the wrong time.

Worst case with 100 kΩ, starting on a weak battery at 5 V:
`VCC = 5 − (750 µA + 140 µA) × 220 Ω = 4.80 V`, comfortably over the 4 V
minimum.

### The pull-up is also the top of the LM5155 EN divider

One network does both jobs — ENOUT ties to the divider junction, grounding it
when asserted and letting it divide when released:

```
VCC ──[ R_top 100 kΩ ]──┬── LM5155-Q1 EN
                        ├── ENOUT
                     [ R_bot 51.1 kΩ ]
                        │
                       GND
...
```
at 4.4 V in   EN = 1.49 V    above the ~1.26 V precision threshold ✓
at 13.8 V     EN = 4.67 V
at 27 V clamp EN = 9.13 V    ← the number to check
```

**[verify] the LM5155-Q1 EN pin's absolute maximum against 9.13 V.** If it is
lower, shift the ratio down and accept less margin at the 4.4 V floor, or clamp
EN with a small Zener. The divider can also double as the SEPIC's own input
UVLO, though the LTC4364's 4.47 V UV already covers that.
...
### Pull-up: to the OUT rail, not 3.3 V

The 3.3 V rail is generated by the SEPIC that ENOUT enables, so it cannot be
the pull-up for its own enable signal. Pull ENOUT to the **LTC4364 OUT rail**
through a resistor. ENOUT withstands 80 V, so the 27 V clamp is no problem for
that pin.

**[verify] the LM5155-Q1 EN pin's absolute maximum**, which is the part that
does need care — EN sees the same rail and will need a divider if it is not
rated to 27 V. The LM5155's precision enable threshold (~1.26 V) makes a divider
natural anyway, and it can double as the SEPIC's own input UVLO.
